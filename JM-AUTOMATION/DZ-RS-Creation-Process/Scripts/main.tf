...
# Get existing public DNS zone information
data "azurerm_dns_zone" "existing" {
  name                = "digital.matthey.com"
  resource_group_name = "rg-hub-eu-publicdns-001"
}

# Create a new A record set in the fetched DNS zone
resource "azurerm_dns_a_record" "record_a" {
...
# Create a new CNAME record set in the fetched DNS zone
resource "azurerm_dns_cname_record" "record_cname" {
